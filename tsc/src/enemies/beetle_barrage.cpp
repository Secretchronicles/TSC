...
void cBeetleBarrage::Init()
{
	...
	m_beetle_fly_speed = -8.0f;
	m_beetle_rand_direction = 2.0f;
	m_beetle_spit_count = 5;
	m_beetle_spawned_count = 0;
	m_beetle_spawn_interval = 10.0f;
	...
	// play the animation when the player is near.
	Set_Animation_Image_Range(0, 1);
	Set_Time_All(300, true);
	Reset_Animation();
	Set_Image_Num(0);
	Set_Animation(false);
...
void cBeetleBarrage::Update()
{
	...
	if (m_active_area.Intersects(pLevel_Player->m_col_rect)) {
		...
		m_is_spitting_out_beetles = false;
		m_beetle_spawned_count = 0;
		m_beetle_interval_counter = 0;
		m_spitting_beetles_counter = 0;
		Set_Animation(false);
		Set_Time_All(300, true);
		Set_Image_Num(0);
		Reset_Animation();
	...
	if (m_is_spitting_out_beetles) {
		if ((m_spitting_beetles_counter <= m_beetle_spawn_interval/3) && (m_beetle_spawned_count < m_beetle_spit_count)) {
			Set_Animation(false);
			Set_Image_Num(2);
		}
		if (m_spitting_beetles_counter <= 0.0f) {
			m_spitting_beetles_counter = 0.0f;
			if (m_beetle_spawned_count < m_beetle_spit_count)
				Generate_Beetle();
			else {
				m_is_spitting_out_beetles = false;
				m_beetle_spawned_count = 0;
...
void cBeetleBarrage::Generate_Beetle()
{
	Scripting::cSpit_Event evt;
	evt.Fire(pActive_Level->m_mruby, this);

	m_beetle_spawned_count++;
	m_spitting_beetles_counter = m_beetle_spawn_interval;
	Reset_Animation();
	Set_Image_Num(0);
	...
	p_beetle->Do_Beetle_Barrage_Generation(m_beetle_fly_distance, m_beetle_fly_speed, m_beetle_rand_direction);
